...
// API pública para obtener causas
app.get('/api/causes', async (req, res) => {
  try {
    const { data: causes, error } = await supabase
      ...
      .select(`
        *,
        causes_members!inner(role, user_id)
      `)
      .eq('status', 'active')
      .order('created_at', { ascending: false });
    ...
    // Procesar causas para incluir información de donaciones
    const processedCauses = causes.map(cause => ({
      ...cause,
      donors: cause.causes_members?.filter(m => m.role === 'donor').length || 0,
      raised: 0 // Calcular desde donaciones reales si tienes esa tabla
    }));

    res.json(processedCauses);

  } catch (error) {
    console.error('Error loading causes:', error);
    res.status(500).json({ error: 'Error cargando causas' });
  }
});