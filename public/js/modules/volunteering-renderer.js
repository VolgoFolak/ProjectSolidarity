...
class VolunteeringRenderer {
  ...
  renderGrid(volunteerings, container) {
    ...
    // ✅ GUARDAR DATOS GLOBALMENTE PARA MODAL ADMINISTRAR (nueva línea)
  ...
  attachEventListeners(container) {
    ...
    // ✅ EVENT LISTENER PARA BOTÓN ADMINISTRAR (nuevo bloque completo)
    container.querySelectorAll('.admin-activity-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        console.log('🔧 Botón administrar clickeado:', { activityId, activityType });
        
        if (typeof window.openAdminActivityModal === 'function') {
          const volunteering = window.volunteerings?.find(v => v.id == activityId);
          if (volunteering) {
            console.log('✅ Abriendo modal admin para voluntariado:', volunteering);
            window.openAdminActivityModal(volunteering);
          } else {
            console.error('❌ No se encontró el voluntariado:', activityId);
          }
        } else if (typeof window.openAdminModal === 'function') {
          // Fallback para compatibilidad
          const volunteering = window.volunteerings?.find(v => v.id == activityId);
          if (volunteering) {
            console.log('✅ Abriendo modal admin (fallback) para voluntariado:', volunteering);
            window.openAdminModal(volunteering);
          }
        } else {
          console.error('❌ No se encontró función openAdminActivityModal ni openAdminModal');
        }
      });
    });
