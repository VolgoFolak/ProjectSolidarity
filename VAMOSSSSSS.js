...
app.post('/login-supabase', async (req, res) => {
  ...
  console.log('- Token preview:', token?.substring(0, 30) + '...' || 'NULL');
  ...
  try {
    ...
    // 🔍 AGREGAR ESTOS LOGS ESPECÍFICOS:
    console.log('🔍 Searching for profile with user ID:', user.id);
    
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('id, username, first_name, email, photo_url') // ❌ Quitar user_type
      .eq('id', user.id)
      .single();
    ...
    req.session.regenerate((err) => {
      ...
      req.session.user = {
        ...
        email: profile.email,
        type: 'user' // ✅ Valor por defecto sin consultar la columna
      };
      req.session.lastActivity = new Date();
...
async function verifyStripeStatus(userId) {
  try {
    ...
    // Cache de 5 minutos para evitar llamadas excesivas a Stripe
  ...
  try {
    ...
    if (!req.session.user) {
      const { data: user, error: userError } = await supabase
        .from('profiles')
        .select('id, username, email, photo_url, first_name') // ❌ Quitar user_type
        .eq('id', user_id)
        .single();
...
const renderView = (view, title = '') => (req, res) => {
  res.render(view, { 
    ...
    env: {
      ...
      SUPABASE_KEY: process.env.SUPABASE_KEY, // <-- CORRECTO
...
// ✅ MIDDLEWARE DE SINCRONIZACIÓN DE SESIÓN
app.use((req, res, next) => {
  // Agregar headers para sincronización con frontend
  res.set({
...
// ✅ ENDPOINT DE HEARTBEAT MÁS ROBUSTO
...
// ✅ MIDDLEWARE PARA DETECTAR CAMBIOS DE SESIÓN
app.use((req, res, next) => {
  const originalJson = res.json;
  res.json = function(data) {
    // Agregar metadatos de sesión a todas las respuestas JSON
