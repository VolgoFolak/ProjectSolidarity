...
// Endpoint para obtener clave pública
...
const authenticateUser = (req, res, next) => {
  ...
  if (req.body.userId && req.body.userId !== req.session.user.id) {
...
app.post('/create-stripe-account', authenticateUser, async (req, res) => {
  try {
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      return_url: `${process.env.FRONTEND_URL}/causes?stripe=success&user_id=${userId}`,
...
app.post('/stripe-webhook', bodyParser.raw({type: 'application/json'}), async (req, res) => {
  ...
  try {
    ...
    console.error(`⚠️ Firma webhook inválida: ${err.message}`);
    return res.status(400).send(`Firma webhook inválida: ${err.message}`);
  }
  try {
    switch (event.type) {
...
async function handleAccountUpdate(account) {
  ...
  await supabase
    ...
    .update({ status, details_submitted: account.details_submitted })
    .eq('stripe_account_id', account.id);
}

async function handleCheckoutCompletion(session) {
  // Lógica para manejar sesión completada
}
