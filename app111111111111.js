...
if (!isLocalhost) {
  app.set('trust proxy', 1); // Para producción detrás de proxy/HTTPS
}
...
app.post('/api/session/keep-alive', (req, res) => {
  ...
  if (req.body.currentPath?.includes('/causes')) {
    verifyStripeStatus(req.session.user.id)
      .then(status => {
        res.json({ ok: true, stripeStatus: status });
      })
      .catch(error => {
...
app.get('/stripe-callback', async (req, res) => {
  try {
    ...
    const returnUrl = req.cookies.stripe_return_url || '/profile';
    ...
    if (user_id && !req.session.user) {
      ...
      if (userError || !user) {
        console.error('Error al restaurar usuario:', userError);
    ...
    async function completeStripeCallback() {
      try {
        const { data: stripeAccount } = await supabase
          .from('stripe_accounts')
          .select('stripe_account_id, charges_enabled')
    ...
    res.redirect('/error?message=stripe_callback_failed');
  }
});